...
"""Script to test partial assemblies simulation using IsaacLab framework."""
...
parser = argparse.ArgumentParser(description="Test partial assemblies simulation.")
parser.add_argument("--num_envs", type=int, default=1, help="Number of environments to simulate.")
parser.add_argument("--task", type=str, default="OmniReset-PartialAssemblies-v0", help="Name of the task.")
...
def main(env_cfg, agent_cfg) -> None:
    """Main function to run partial assemblies simulation."""
    ...
    try:
        while True:
            # Step environment (physics simulation with external forces)
            _, rewards, terminated, truncated, info = env.step(actions)
            dones = terminated | truncated

            # Print insertive object state periodically to show effect of forces
