...
from ....common.vision import VISION_RESOURCES_DIR
from ... import mdp as task_mdp

from .rl_state_cfg import RlStateSceneCfg, BaseEventCfg, Ur5eRobotiq2f85RelCartesianOSCEvalCfg
...
@configclass
class BaseRGBEventCfg(BaseEventCfg):
...
class RGBEventCfg(BaseRGBEventCfg):
    ...
    # reset visual appearance (texture or color)
    randomize_wrist_mount_appearance = EventTerm(
        ...
        params={
            ...
            "texture_prob": 0.05,
            "texture_config_path": str(VISION_RESOURCES_DIR / "texture_paths.yaml"),
    ...
    randomize_sky_light = EventTerm(
        ...
        params={
            ...
            "hdri_config_path": str(VISION_RESOURCES_DIR / "hdri_paths.yaml"),
...
@configclass
class TrainRGBEventCfg(RGBEventCfg):
    """Configuration for data collection: visual randomizations + reset-from-states."""
...
@configclass
class EvalRGBEventCfg(RGBEventCfg):
...
class RGBCommandsCfg:
    ...
    task_command = task_mdp.TaskCommandCfg(
        ...
        success_position_threshold=0.01,  # looser threshold for position since harder to tell from rgb
        success_orientation_threshold=float("inf"),  # only check position; policy can't tell orientation from rgb
    )
...
class RGBObservationsCfg:
    ...
    class RGBDataCollectionCfg(ObsGroup):
        ...
        # Auxiliary obs for sim2real (recorded in sim, used as reconstruction targets in training)
...
class DataCollectionRGBTerminationsCfg:
    ...
    corrupted_camera = DoneTerm(
        ...
        params={
            "camera_names": ["front_camera", "side_camera", "wrist_camera"],
            "std_threshold": 10.0
        }
    )
...
@configclass
class Ur5eRobotiq2f85RGBRelCartesianOSCEvalCfg(Ur5eRobotiq2f85RelCartesianOSCEvalCfg):
    """RGB base config: finetune physics + RGB scene/obs/terminations/render."""
    scene: DataCollectionRGBObjectSceneCfg = DataCollectionRGBObjectSceneCfg(num_envs=32, env_spacing=1.5, replicate_physics=False)
...
class Ur5eRobotiq2f85DataCollectionRGBRelCartesianOSCCfg(Ur5eRobotiq2f85RGBRelCartesianOSCEvalCfg):
    events: TrainRGBEventCfg = TrainRGBEventCfg()


@configclass
class Ur5eRobotiq2f85EvalRGBRelCartesianOSCCfg(Ur5eRobotiq2f85RGBRelCartesianOSCEvalCfg):
    """Evaluation config for Cartesian OSC delta actions. No obs corruption for stable policy eval."""

    events: EvalRGBEventCfg = EvalRGBEventCfg()
...
class OODRGBEventCfg(BaseRGBEventCfg):
    ...
    randomize_wrist_mount_appearance = EventTerm(
        ...
        params={
            ...
            "mesh_names": ["robotiq_base_link/visuals/D415_to_Robotiq_Mount"],
            "texture_prob": 0.05,
            "texture_config_path": str(VISION_RESOURCES_DIR / "texture_paths_ood.yaml"),
            "diffuse_tint_range": ((0.0, 0.0, 0.0), (1.0, 1.0, 1.0)),
            "colors": {"r": (0.0, 1.0), "g": (0.0, 1.0), "b": (0.0, 1.0)},
        },
    )
    ...
    randomize_sky_light = EventTerm(
        ...
        params={
            "light_path": "/World/skyLight",
            "hdri_config_path": str(VISION_RESOURCES_DIR / "hdri_paths_ood.yaml"),
            "intensity_range": (1000.0, 4000.0),
            "rotation_range": (0.0, 360.0),
        },
    )
...
@configclass
class TrainOODRGBEventCfg(OODRGBEventCfg):
    """Configuration for training events with OOD (out-of-distribution) textures and HDRIs."""
...
@configclass
class EvalOODRGBEventCfg(OODRGBEventCfg):
    """Configuration for evaluation events with OOD (out-of-distribution) textures and HDRIs."""
...
class Ur5eRobotiq2f85DataCollectionRGBRelCartesianOSCOODCfg(Ur5eRobotiq2f85DataCollectionRGBRelCartesianOSCCfg):
    ...
    events: TrainOODRGBEventCfg = TrainOODRGBEventCfg()
...
class Ur5eRobotiq2f85EvalRGBRelCartesianOSCOODCfg(Ur5eRobotiq2f85EvalRGBRelCartesianOSCCfg):
    ...
    events: EvalOODRGBEventCfg = EvalOODRGBEventCfg()
