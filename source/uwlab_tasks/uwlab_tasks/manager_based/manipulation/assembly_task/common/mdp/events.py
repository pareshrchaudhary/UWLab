...
class MultiResetManager(ManagerTermBase):
    """Loads multiple reset-state datasets and samples from them at each reset."""
    ...
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        ...
        dataset_files = [f"{base_path}/{reset_state_hash}.pt" for base_path in base_paths]
        ...
        for dataset_file in dataset_files:
            ...
            self.datasets.append(utils.sample_state_data_set(dataset, init_indices, env.device))
    ...
    def __call__(
        ...
        for dataset_idx in range(self.num_tasks):
            ...
            states_to_reset_from = utils.sample_from_nested_dict(self.datasets[dataset_idx], state_indices)
