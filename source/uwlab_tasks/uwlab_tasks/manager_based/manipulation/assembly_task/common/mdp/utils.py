...
def load_asset_paths_from_config(
    ...
    1. Local: if ``UWLAB_LOCAL_ASSETS_DIR`` is set and the file exists there, use it.
    2. Cloud: else resolve against ``UWLAB_CLOUD_ASSETS_DIR`` (B2).
    3. Nvidia cloud: paths under ``nvidia_cloud:`` resolve against ``NVIDIA_CLOUD_ASSETS_DIR``.

    The YAML should have ``cloud:`` and optionally ``nvidia_cloud:`` keys with
    lists of relative paths (e.g. ``Assets/PolyHaven/...``).
    ...
    Returns:
        List of resolved absolute paths (local paths or cloud URLs).
    """
    from uwlab_assets import NVIDIA_CLOUD_ASSETS_DIR, UWLAB_CLOUD_ASSETS_DIR

    local_base = os.environ.get("UWLAB_LOCAL_ASSETS_DIR")
    if not local_base:
        for p in Path(__file__).resolve().parents:
            if p.name == "source" and (p.parent / "resources").is_dir():
                local_base = str(p.parent / "resources")
                break
    local_base = os.path.abspath(local_base) if local_base else None

    with open(config_path, "r") as f:
        config = yaml.safe_load(f)

    def resolve_path(rel: str, cloud_base: str) -> str:
        if local_base:
            candidate = os.path.join(local_base, rel)
            if os.path.exists(candidate):
                return candidate
        return os.path.join(cloud_base, rel)

    paths = []
    for relative_path in config.get("cloud", []):
        if isinstance(relative_path, str) and relative_path.strip().startswith("Assets/"):
            paths.append(resolve_path(relative_path.strip(), UWLAB_CLOUD_ASSETS_DIR))
    for relative_path in config.get("nvidia_cloud", []):
        if isinstance(relative_path, str) and relative_path.strip().startswith("Assets/"):
            paths.append(resolve_path(relative_path.strip(), NVIDIA_CLOUD_ASSETS_DIR))
